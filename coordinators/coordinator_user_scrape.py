...
from scrapers.user_scraper import fetch_user, fetch_fav_tracks, save_user, save_fav_tracks
...
def run_user_scrape():
    ...
    print(f"[User] ▶️  Tiếp tục từ user_id {start_id} | Đang có {total_db} users trong DB")
    ...
    try:
        while running:
            try:
                ...
                if running:
                    print(f"[User]   ⚠️  Lỗi user_id {user_id}: {e}")
                conn.rollback()
        ...
        """, (user_id,))
        conn.commit()
        print(f"[User] 💾 Đã lưu tiến độ tại user_id {user_id}")
        print(f"[User] ✅ Phiên này: {session_found} users | Tổng DB: {total_db}")
        cur.close()
        conn.close()