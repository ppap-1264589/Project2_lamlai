...
def fetch_fav_tracks(user_id: int) -> list[dict]:
    """
    Lấy toàn bộ danh sách track yêu thích của user, duyệt qua phân trang.
    API trả đủ title/duration/rank nên INSERT thẳng vào bảng tracks được.
    """
    tracks = []
...
def save_fav_tracks(cur, user_id: int, tracks: list[dict]):
    """
    1. INSERT track vào bảng tracks (đủ dữ liệu từ API).
    2. INSERT quan hệ user ↔ track vào user_fav_tracks.
    3. INSERT quan hệ album ↔ track vào album_tracks với track_position = NULL.
    """
    if not tracks:
        return
    ...
    # User API không trả track_position; album scrape sẽ điền position thật sau.
    if album_track_rows:
        album_ids = [(album_id,) for album_id in {row[0] for row in album_track_rows}]
